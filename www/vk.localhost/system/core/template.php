...
// ========================================================================== //
  ...
  public function printJavascriptTags() {
    ...
    if (!$this->site_config->merge_js) {
      foreach ($this->head_main_js as $id => $file) {
        echo "\t" . $this->getJSTag($file) . "\n";
      }
      foreach ($this->head_js as $id => $file) {
        echo "\t" . $this->getJSTag($file) . "\n";
      }
    }
    else {
      ...
      echo $tag;
      foreach ($this->head_js_no_merge as $id => $file) {
        echo "\t" . $this->getJSTag($file) . "\n";
      }
    }
  }
  ...
  public function printCssTags() {
    ...
    if (!$this->site_config->merge_css) {
      foreach ($this->head_main_css as $id => $file) {
        echo "\t" . $this->getCSSTag($file) . "\n";
      }
      foreach ($this->head_css as $id => $file) {
        echo "\t" . $this->getCSSTag($file) . "\n";
      }
    }
    else {
      ...
      echo $tag;
      foreach ($this->head_css_no_merge as $id => $file) {
        echo "\t" . $this->getCSSTag($file) . "\n";
      }
    }
  }
...
// ========================================================================== //
  ...
  public function setPageTitle($pagetitle) {
    if (func_num_args() > 1) {
      $pagetitle = implode(' · ', func_get_args());
    }
...
//============================================================================//
  ...
  public function render($tpl_file, $data = array(), $request = false) {
    ...
    if ($css_file) {
      $this->addCSSFromContext($css_file, $request);
    }
  ...
  public function renderGridRowsJSON($grid, $dataset, $total = 1, $pages_count = 1) {
    ...
    //
      ...
      foreach ($dataset as $row) {
        ...
        $cell_index     = 0;
        $editable_index = 1;
        $editable_count = count(array_filter($grid['columns'], function($element) {
            return isset($element['editable']);
          }));
